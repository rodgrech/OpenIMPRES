/*
  OpenIMPRES V5 DS2433 Read Map

  READ ONLY / PASSIVE SNIFFER

  V5 keeps the reset-delimited passive capture flow and adds a compact
  DS2433-focused summary. It does not drive the bus. It watches for:

    Match ROM (55) -> DS2433 family (A3) -> Read Memory/Search opcode (F0)

  The sketch still prints the raw decoded transaction bytes, but also emits a
  ds2433_reads array so repeated bench captures can be compared by ROM/address.
*/
...
#define MAX_PULSES 18000
#define MAX_TRANSACTIONS 128
#define MAX_TX_BYTES 48
#define PRINT_TRANSACTIONS false
...
void printBytesFrom(const Transaction &tx, uint8_t start) {
  for (uint8_t i = start; i < tx.byteCount; i++) {
...
bool isDs2433Read(const Transaction &tx) {
  return tx.byteCount >= 12 &&
         tx.bytes[0] == ONEWIRE_MATCH_ROM &&
         tx.bytes[1] == DS2433_FAMILY &&
         tx.bytes[9] == ONEWIRE_SEARCH_OR_READ_MEMORY;
}

bool isDs2438Match(const Transaction &tx) {
  return tx.byteCount >= 9 &&
         tx.bytes[0] == ONEWIRE_MATCH_ROM &&
         tx.bytes[1] == DS2438_FAMILY;
}

uint16_t readAddress(const Transaction &tx) {
  return ((uint16_t)tx.bytes[11] << 8) | tx.bytes[10];
}

uint16_t countDs2433Reads() {
  uint16_t count = 0;
  for (uint16_t i = 0; i < transactionCount; i++) {
    if (isDs2433Read(transactions[i])) count++;
  }
  return count;
}

uint16_t countDs2438Matches() {
  uint16_t count = 0;
  for (uint16_t i = 0; i < transactionCount; i++) {
    if (isDs2438Match(transactions[i])) count++;
  }
  return count;
}

uint16_t countPrintableTransactions() {
  if (PRINT_TRANSACTIONS) return transactionCount;
  uint16_t count = 0;
  for (uint16_t i = 0; i < transactionCount; i++) {
    if (isDs2433Read(transactions[i]) || isDs2438Match(transactions[i])) count++;
  }
  return count;
}
...
void printDs2433Read(const Transaction &tx, uint16_t index) {
  Serial.println("    {");
  ...
  Serial.print("      \"rom\": \""); printRomFrom(tx, 1); Serial.println("\",");
  Serial.print("      \"op\": \""); Serial.print(hexByte(tx.bytes[9])); Serial.println("\",");
  Serial.print("      \"op_name\": \""); Serial.print(opName(tx.bytes[9])); Serial.println("\",");
  Serial.print("      \"address_le\": \""); printAddress(readAddress(tx)); Serial.println("\",");
  Serial.print("      \"tail_byte_count\": "); Serial.print(tx.byteCount > 12 ? tx.byteCount - 12 : 0); Serial.println(",");
  Serial.print("      \"tail_bytes\": \""); printBytesFrom(tx, 12); Serial.println("\",");
...
void printDs2438Match(const Transaction &tx, uint16_t index) {
  Serial.println("    {");
  Serial.print("      \"index\": "); Serial.print(index); Serial.println(",");
  Serial.print("      \"reset_pulse_index\": "); Serial.print(tx.resetPulseIndex); Serial.println(",");
  Serial.print("      \"rom\": \""); printRomFrom(tx, 1); Serial.println("\",");
  Serial.print("      \"op\": \"");
  if (tx.byteCount >= 10) Serial.print(hexByte(tx.bytes[9]));
  Serial.println("\",");
  Serial.print("      \"op_name\": \"");
  if (tx.byteCount >= 10) Serial.print(opName(tx.bytes[9]));
  Serial.println("\",");
  Serial.print("      \"bytes\": \""); printBytes(tx); Serial.println("\"");
  Serial.print("    }");
}

void printTransaction(const Transaction &tx, uint16_t index) {
  Serial.println("    {");
  Serial.print("      \"index\": "); Serial.print(index); Serial.println(",");
  Serial.print("      \"reset_pulse_index\": "); Serial.print(tx.resetPulseIndex); Serial.println(",");
...
void printJson() {
  ...
  Serial.println("{");
  Serial.println("  \"sketch\": \"OpenIMPRES_V5_DS2433_Read_Map\",");
  ...
  Serial.print("  \"ds2433_read_count\": "); Serial.print(countDs2433Reads()); Serial.println(",");
  Serial.print("  \"ds2438_match_count\": "); Serial.print(countDs2438Matches()); Serial.println(",");
  Serial.print("  \"print_transactions\": ");
  Serial.print(PRINT_TRANSACTIONS ? "true" : "false");
  Serial.println(",");

  Serial.println("  \"ds2433_reads\": [");
  bool firstPrinted = true;
  for (uint16_t i = 0; i < transactionCount; i++) {
    if (!isDs2433Read(transactions[i])) continue;
    if (!firstPrinted) Serial.println(",");
    firstPrinted = false;
    printDs2433Read(transactions[i], i);
  }
  Serial.println();
  Serial.println("  ],");

  Serial.println("  \"ds2438_matches\": [");
  firstPrinted = true;
  for (uint16_t i = 0; i < transactionCount; i++) {
    if (!isDs2438Match(transactions[i])) continue;
    if (!firstPrinted) Serial.println(",");
    firstPrinted = false;
    printDs2438Match(transactions[i], i);
  }
  Serial.println();
  Serial.println("  ],");
  ...
  firstPrinted = true;
  for (uint16_t i = 0; i < transactionCount; i++) {
    if (!PRINT_TRANSACTIONS && !isDs2433Read(transactions[i]) && !isDs2438Match(transactions[i])) continue;
    if (!firstPrinted) Serial.println(",");
    firstPrinted = false;
...
void setup() {
  ...
  Serial.println();
  Serial.println("OpenIMPRES V5 DS2433 Read Map");
