/*
  OpenIMPRES V3 Reset Transaction Profiler

  READ ONLY / PASSIVE SNIFFER

  V2 proved the capture has strong 1-Wire-like timing. V3 segments the capture
  around reset-like low pulses so each possible 1-Wire transaction can be
...
const char *oneWireHint(const Transaction &tx) {
  ...
  if (tx.bytes[0] == ONEWIRE_SEARCH_ROM) return "SEARCH_ROM_OR_READ_MEMORY";
...
uint8_t countInterestingTransactions() {
  uint8_t count = 0;
...
void printJson() {
  ...
  Serial.println("{");
  Serial.println("  \"sketch\": \"OpenIMPRES_V3_Reset_Transaction_Profiler\",");
...
void setup() {
  ...
  Serial.println();
  Serial.println("OpenIMPRES V3 Reset Transaction Profiler");
